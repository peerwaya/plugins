...
@implementation FlutterVideoCache
...
+ (FlutterVideoCache *)sharedInstance {
    static FlutterVideoCache *sharedInstance = nil;
...
- (id)init {
    if (self = [super init]) {
        self.cacheIdentifier = @"Flutter.video.cache";
...
- (NSString *)generateCacheKeyForUri:(NSString *)uri {
    ...
    if ([pathExtension isEqualToString:@""]) {
        ...
        NSError *error = [NSError errorWithDomain:@"FlutterVideoCache"
                                             code:FlutterVideoCacheStatusMissingFileExtension userInfo:userInfo];
        @throw error;
        ...
        NSError *error = [NSError errorWithDomain:@"FlutterVideoCache"
                                             code:FlutterVideoCacheStatusUnsupportedFileExtension userInfo:userInfo];
        @throw error;
    }
    return [[self generateHashForUrl:uri] stringByAppendingPathExtension:pathExtension];
}

- (void)getItemForUri:(NSString *)uri withCallback:(void(^)(FlutterVideoCacheStatus, AVAsset * _Nullable)) handler {
    @try {
        NSString *key = [self generateCacheKeyForUri:uri];
        AVURLAsset * temporaryAsset = [self getItemFromTemporaryStorage:key];
        if (temporaryAsset != nil) {
            handler(FlutterVideoCacheStatusAvailable, temporaryAsset);
            return;
        }
        ...
        [self.videoCache loadDataForKey:key withCallback:^(SPTPersistentCacheResponse * _Nonnull response) {
            if (response.record == nil || response.record.data == nil) {
                handler(FlutterVideoCacheStatusNotAvailable, nil);
                return;
            }
            [self saveDataToTemporaryStorage:response.record.data key:key];
            handler(FlutterVideoCacheStatusAvailable, [self getItemFromTemporaryStorage:key]);
        } onQueue:dispatch_get_main_queue()];
    } @catch (NSError * err) {
        switch (err.code) {
            case FlutterVideoCacheStatusMissingFileExtension:
                handler(FlutterVideoCacheStatusMissingFileExtension, nil);
                return;
            case FlutterVideoCacheStatusUnsupportedFileExtension:
                handler(FlutterVideoCacheStatusUnsupportedFileExtension, nil);
                return;
            default:
                @throw err;
        }
