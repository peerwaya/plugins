...
- (BOOL)setupTexturesForFrame:(nonnull CVPixelBufferRef)frame rotation:(int)rotation {
  ...
  // Luma (y) texture.
  int lumaWidth = CVPixelBufferGetWidthOfPlane(frame, 0);
  int lumaHeight = CVPixelBufferGetHeightOfPlane(frame, 0);

  int indexPlane = 0;
  CVReturn result = CVMetalTextureCacheCreateTextureFromImage(
      kCFAllocatorDefault, _textureCache, frame, nil, MTLPixelFormatR8Unorm, lumaWidth,
  ...
  result = CVMetalTextureCacheCreateTextureFromImage(
      kCFAllocatorDefault, _textureCache, frame, nil, MTLPixelFormatRG8Unorm, lumaWidth / 2,
